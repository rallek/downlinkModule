...
abstract class AbstractThirdPartyListener implements EventSubscriberInterface
{
    ...
    public function contentGetTypes(\Zikula_Event $event)
    {
        ...
        $types->add('RKDownlinkModule_ContentType_Item');
        
        // plugin for showing a list of multiple items
        $types->add('RKDownlinkModule_ContentType_ItemList');
    }
     ...
     * RKDownlinkModule will use this to add a javascript helper to add custom items.
     *
     * You can access general data available in the event.
     *
     * The event name:
    ...
    public function getEditorHelpers(EditorHelperEvent $event)
    {
        ...
        $targetDir = 'web/modules/rkdownlink';
        $finder = new Finder();
        if (!$this->filesystem->exists($targetDir)) {
            $this->filesystem->mkdir($targetDir, 0777);
            if (is_dir($originDir = 'modules/RK/DownlinkModule/Resources/public')) {
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
            if (is_dir($originDir = 'modules/RK/DownlinkModule/Resources/scribite')) {
                $targetDir .= '/scribite';
                $this->filesystem->mkdir($targetDir, 0777);
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
        ...
        $event->getHelperCollection()->add(
            [
                ...
                'type' => 'javascript',
                'path' => $this->request->getBasePath() . '/web/modules/rkdownlink/js/RKDownlinkModule.Finder.js'
            ]
        );
    }
    ...
    public function getCKEditorPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rkdownlinkmodule',
            'path' => $this->request->getBasePath() . '/web/modules/rkdownlink/scribite/CKEditor/rkdownlinkmodule/',
            'file' => 'plugin.js',
            'img' => 'ed_rkdownlinkmodule.gif'
        ]);
    }
    ...
    public function getQuillPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rkdownlinkmodule',
            'path' => $this->request->getBasePath() . '/web/modules/rkdownlink/scribite/Quill/rkdownlinkmodule/plugin.js'
        ]);
    }
    ...
    public function getSummernotePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rkdownlinkmodule',
            'path' => $this->request->getBasePath() . '/web/modules/rkdownlink/scribite/Summernote/rkdownlinkmodule/plugin.js'
        ]);
    }
    ...
    public function getTinyMcePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rkdownlinkmodule',
            'path' => $this->request->getBasePath() . '/web/modules/rkdownlink/scribite/TinyMce/rkdownlinkmodule/plugin.js'
        ]);
    }
}
