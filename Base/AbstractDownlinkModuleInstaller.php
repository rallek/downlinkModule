...
abstract class AbstractDownlinkModuleInstaller extends AbstractExtensionInstaller
{
    ...
    */
    ...
    protected function updateModVarsTo14()
    {
        ...
        $conn->update('module_vars', ['modname' => 'RKDownlinkModule'], ['modname' => 'Downlink']);
    }
    ...
    {
        $conn = $this->getConnection();
        $conn->update('modules', ['name' => 'RKDownlinkModule', 'directory' => 'RK/DownlinkModule'], ['name' => 'Downlink']);
    }
    ...
    {
        $conn = $this->getConnection();
        $componentLength = strlen('Downlink') + 1;
    
        $conn->executeQuery("
            UPDATE group_perms
            SET component = CONCAT('RKDownlinkModule', SUBSTRING(component, $componentLength))
            WHERE component LIKE 'Downlink%';
        ");
    }
    ...
    protected function renameCategoryRegistriesFor14()
    {
        ...
        $conn->executeQuery("
            ...
            SET modname = CONCAT('RKDownlinkModule', SUBSTRING(modname, $componentLength))
            WHERE modname LIKE 'Downlink%';
        ");
    }
    ...
    {
        \EventUtil::unregisterPersistentModuleHandlers('Downlink');
    }
    ...
    protected function updateHookNamesFor14()
    {
        ...
        $conn->update('hook_area', ['owner' => 'RKDownlinkModule'], ['owner' => 'Downlink']);
    
        $componentLength = strlen('subscriber.downlink') + 1;
        $conn->executeQuery("
            UPDATE hook_area
            SET areaname = CONCAT('subscriber.rkdownlinkmodule', SUBSTRING(areaname, $componentLength))
            WHERE areaname LIKE 'subscriber.downlink%';
        ");
    
        $conn->update('hook_binding', ['sowner' => 'RKDownlinkModule'], ['sowner' => 'Downlink']);
    
        $conn->update('hook_runtime', ['sowner' => 'RKDownlinkModule'], ['sowner' => 'Downlink']);
    
        $componentLength = strlen('downlink') + 1;
        $conn->executeQuery("
            UPDATE hook_runtime
            SET eventname = CONCAT('rkdownlinkmodule', SUBSTRING(eventname, $componentLength))
            WHERE eventname LIKE 'downlink%';
        ");
    
        $conn->update('hook_subscriber', ['owner' => 'RKDownlinkModule'], ['owner' => 'Downlink']);
    ...
    {
        $conn = $this->getConnection();
        $conn->update('workflows', ['module' => 'RKDownlinkModule'], ['module' => 'Downlink']);
        $conn->update('workflows', ['obj_table' => 'LinkEntity'], ['module' => 'RKDownlinkModule', 'obj_table' => 'link']);
    }
