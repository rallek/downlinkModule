...
namespace RK\DownlinkModule\ContentType\Base;
...
abstract class AbstractItemList extends \Content_AbstractContentType implements ContainerAwareInterface
{
    ...
    {
        return $this->container->get('translator.default')->__('RKDownlinkModule list view');
    }
    ...
    {
        return $this->container->get('translator.default')->__('Display list of RKDownlinkModule objects.');
    }
    ...
    {
        $controllerHelper = $this->container->get('rk_downlink_module.controller_helper');
        ...
        $featureActivationHelper = $this->container->get('rk_downlink_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $this->objectType)) {
            $this->categorisableObjectTypes = ['link'];
            $categoryHelper = $this->container->get('rk_downlink_module.category_helper');
    
            // fetch category properties
            $this->catRegistries = [];
            $this->catProperties = [];
            if (in_array($this->objectType, $this->categorisableObjectTypes)) {
                $entityFactory = $this->container->get('rk_downlink_module.entity_factory');
    ...
    {
        $repository = $this->container->get('rk_downlink_module.entity_factory')->getRepository($this->objectType);
        $permissionApi = $this->container->get('zikula_permissions_module.api.permission');
    
        // create query
        $orderBy = $this->container->get('rk_downlink_module.model_helper')->resolveSortParameter($this->objectType, $this->sorting);
        ...
        $templateParameters = $this->container->get('rk_downlink_module.controller_helper')->addTemplateParameters($this->objectType, $templateParameters, 'contentType', []);
    ...
    protected function getDisplayTemplate()
    {
        ...
        foreach ($templateOptions as $templatePath) {
            if ($templating->exists('@RKDownlinkModule/' . $templatePath)) {
                $template = '@RKDownlinkModule/' . $templatePath;
                break;
            }
        }
    ...
    public function startEditing()
    {
        ...
        $this->view->toplevelmodule = 'RKDownlinkModule';
    
        // ensure our custom plugins are loaded
        array_push($this->view->plugins_dir, 'modules/RK/DownlinkModule/Resources/views/plugins');
        ...
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $this->objectType)) {
            ...
            $this->view->assign('categories', $categories)
                       ->assign('categoryHelper', $this->container->get('rk_downlink_module.category_helper'));
        }
