...
namespace RK\DownlinkModule\Form\Handler\Link\Base;

use RK\DownlinkModule\Form\Handler\Common\EditHandler;
use RK\DownlinkModule\Form\Type\LinkType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    public function processForm(array $templateParameters = [])
    {
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the link yet as other items are required which must be created before!'));
                $logArgs = ['app' => 'RKDownlinkModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType];
    ...
    {
        return $this->formFactory->create(LinkType::class, $this->entityRef, $this->getFormOptions());
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of link area
        $codes[] = 'userIndex';
        // admin index page of link area
        $codes[] = 'adminIndex';
        // user list of links
        $codes[] = 'userView';
        // admin list of links
        $codes[] = 'adminView';
        // user list of own links
        $codes[] = 'userOwnView';
        // admin list of own links
        $codes[] = 'adminOwnView';
        // user detail page of treated link
        $codes[] = 'userDisplay';
        // admin detail page of treated link
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultReturnUrl(array $args = [])
    {
        ...
        $routePrefix = 'rkdownlinkmodule_' . $this->objectTypeLower . '_' . $routeArea;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'RKDownlinkModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'link', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
    ...
    protected function getRedirectUrl(array $args = [])
    {
        ...
        if ($this->request->getSession()->has('rkdownlinkmodule' . $this->objectTypeCapital . 'Referer')) {
            $this->request->getSession()->remove('rkdownlinkmodule' . $this->objectTypeCapital . 'Referer');
        }
